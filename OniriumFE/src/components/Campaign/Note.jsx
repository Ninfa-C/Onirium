    ...
    <section id="notes" className="mb-12">
      ...
      {expandedSections && (
        <div className="space-y-4">
          {notes
          .filter((note) => campaign.role === "master" || (note.isVisible && !note.masterOnly))
          .map((note) => (
            <Card
              key={note.id}
            ...
            >
              <CardContent className="p-0 flex">
               {note.imageString && <div className="w-24 bg-gray-800 flex-shrink-0 overflow-hidden rounded-l-md">
                  <img
                    src={
                      note.imagePreview
                        ? note.imagePreview
                    ...
                    onClick={() => {setImageModalOpen(true), setImageId(note.id)}}
                  />
                </div>}
                ...
                {campaign.role === "master" &&  <div className="flex justify-end mt-3 gap-2">
                    <Button
                      className={`${
                        note.isVisible
                    ...
                    <Button
                      ...
                      onClick={() => {
                        setUpdateModal(true), setEditingId(note.id);
                      }}
                    >
              ...
              {(editingId == note.id && campaign.role === "master") && (
                <UpdateNote
                  isOpen={updateModal}
